  ...
  if (serviceVisual) {
    ...
    const play = () => {
      ...
      if (!film.getAttribute("src"))
        film.src = SPHERE.assets + "service-motion.mp4";
      film
        .play()
        .then(() => {
    ...
    document.querySelectorAll(".service-row").forEach((row, i) => {
      const change = () => {
        ...
        const stamp = ++ticket;
        const img = new Image();
        img.src = `${SPHERE.assets}${images[i]}`;
        img
          .decode()
          .then(() => {
  ...
  if (selection) {
    ...
    const update = () => {
      ...
      const distance = Math.max(
        1,
        sequence.offsetHeight - innerHeight + 88 - Math.max(600, innerHeight),
      );
...
(() => {
  ...
  menu.querySelectorAll(".menu-links>a").forEach((link, i) => {
    ...
    const update = () => {
      ...
      image.src = `${SPHERE.assets}${link.dataset.preview}-xl.webp`;
      image
        .decode()
        .then(() => {
...
(() => {
  ...
  function update() {
    ...
    const readingHold = Math.max(480, innerHeight * 0.8);
    const distance = Math.max(
      1,
      track.offsetHeight - scene.offsetHeight - readingHold,
    );
...
(() => {
  ...
  const stop = () => {
    ...
    frame = 0;
    target = scrollY;
    document.documentElement.classList.remove("wheel-gliding");
  };
  function tick(time) {
    ...
    scrollTo(0, Math.abs(target - next) < 0.8 ? target : next);
    written = scrollY;
  ...
  addEventListener(
    ...
    (e) => {
      ...
      if (
        e.target.closest(
          'input,textarea,select,[role="dialog"],.editorial-menu.open,.modal.open',
        )
      ) {
        stop();
        return;
      ...
      const delta =
        e.deltaY *
        (e.deltaMode === 1 ? 20 : e.deltaMode === 2 ? innerHeight : 1);
      target += Math.max(-120, Math.min(120, delta * 0.65));
      const ahead = Math.min(200, innerHeight * 0.33);
  ...
  ["pointerdown", "keydown", "touchstart", "blur"].forEach((name) =>
    addEventListener(name, stop, { passive: true }),
  );
  addEventListener(
    "scroll",
    () => {
      if (frame && Math.abs(scrollY - written) > 3) stop();
    },
    { passive: true },
  );
...
(() => {
  ...
  const sources = [
    SPHERE.assets + "portfolio/item-014-sm.webp",
    SPHERE.assets + "portfolio/item-020-sm.webp",
    SPHERE.assets + "portfolio/item-001-sm.webp",
  ];
  ...
  function position(x, y) {
    ...
    preview.style.top = `${Math.max(92, Math.min(innerHeight - size - 16, y - size * 0.5))}px`;
  }
  const hide = () => {
    ticket++;
  ...
  links.forEach((link, i) => {
    const show = async (e) => {
      ...
      position(e.clientX || r.right - 260, e.clientY || r.top + r.height / 2);
      try {
        await asset.decode();
      } catch {
        return;
    ...
    link.addEventListener("pointermove", (e) => position(e.clientX, e.clientY));
...
(() => {
  ...
  button?.addEventListener("click", () => {
    ...
    document
      ...
      .forEach((el) => el.classList.toggle("user-paused", paused));
