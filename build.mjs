...
const services = [
  ...
  [
    "Product rendering",
    "Materiality, form and detail, crafted for marketing and sales.",
    "Product Rendering",
  ],
  [
    "CAD & BIM modeling",
...
const steps = [
  ...
  [
    ...
    "After approvals, final visuals are refined in post-production and delivered in high resolution.",
  ],
];
...
const picture = (id, alt, extra = "") =>
  id.startsWith("portfolio/")
    ...
    : id === "hero-poster"
      ...
      : `<img src="assets/${id}.webp" srcset="assets/${id}-sm.webp 720w, assets/${id}.webp ${id === "insight-1" ? 1448 : id.startsWith("insight-") ? 1672 : 1800}w${["exterior1", "exterior2", "interior1", "interior2", "hospitality"].includes(id) ? `, assets/${id}-xl.webp 2880w` : ""}" sizes="(max-width: 700px) 100vw, 70vw" alt="${alt}" loading="lazy" decoding="async" ${extra}>`;
...
const card = (p, i) =>
  `<button class="project project-${i} reveal" data-project="${projects.indexOf(p)}" data-asset="${p[0]}"${p[4]?.youtube ? ` data-youtube="${p[4].youtube}"` : ""} data-category="${p[1]}" aria-label="Open ${p[2]}"><div class="project-image">${picture(p[0], p[3])}<span class="view">${p[1] === "Animations" ? "Play film" : p[1] === "VR 360°" ? "Explore 360°" : "View image"} ${arrow}</span></div><div class="project-meta"><h3>${p[2].replace(/ \/ \d+$/, "")}</h3>${p[2].startsWith(p[1] + " / ") ? "" : `<span>${p[1]}</span>`}</div></button>`;
const eyebrow = (n, text) =>
  `<p class="eyebrow">${/^\d+$/.test(n) ? "" : `<span>${n}</span>`}${text}</p>`;
const cta = () =>
...
const nav = (active) => {
  ...
  return `<a class="skip" href="#main">Skip to content</a><header class="site-header"><a href="index.html" class="brand" aria-label="Sphere Design home"><img src="assets/logo.png" alt="Sphere Design" width="150" height="52"></a><div class="header-links" aria-label="Quick navigation">${items
    ...
    .join(
      "",
    )}</div><div class="header-actions"><button class="menu-toggle" aria-expanded="false" aria-controls="navigation" aria-label="Open navigation"><span class="menu-word">Menu</span><span class="menu-symbol"><i></i><i></i></span></button></div></header><nav aria-label="Main navigation" id="navigation" class="editorial-menu" aria-hidden="true" inert><div class="menu-layout wrap"><div class="menu-main"><p class="menu-kicker"><span>EXPLORE SPHERE</span></p><div class="menu-links">${items.map(([url, label, img, caption], i) => `<a href="${url}" data-preview="${img}" data-caption="${caption}" ${label === active ? 'aria-current="page"' : ""}><span class="menu-title">${label}</span><span class="menu-arrow" aria-hidden="true">↗</span></a>`).join("")}</div></div><aside class="menu-art" aria-hidden="true"><div class="menu-art-frame"><img src="assets/exterior2-xl.webp" alt="" width="2880" height="1800" loading="lazy"><span class="menu-art-mark">${icon("vr")}</span><span class="menu-art-label">ARCHITECTURE, IMAGINED.</span></div><div class="menu-art-caption"><p>Where the vision begins</p></div></aside><div class="menu-bottom"><span class="menu-location">MIAMI, FLORIDA<span>Working worldwide.</span><button class="background-motion-toggle" type="button" aria-pressed="false">Pause background motion</button><a class="menu-phone" href="tel:+17868844880">+1 (786) 884-4880</a></span><a class="menu-contact" href="mailto:info@thespheredesign.com"><span>HAVE A PROJECT IN MIND?</span>Let’s make it visible. <i aria-hidden="true">↗</i></a><div class="menu-socials">${socialIcon("Instagram", "https://www.instagram.com/thespheredesign/")}${socialIcon("LinkedIn", "https://www.linkedin.com/company/studiospheredesign/")}${socialIcon("Behance", "https://www.behance.net/spheredesignllc")}</div></div></div></nav>`;
};
const footer = () =>
  `<footer class="wrap"><div class="footer-signature" aria-hidden="true"><span>SPHERE</span><i>DESIGN STUDIO — MIAMI & BEYOND</i></div><div class="footer-top"><a class="brand" href="index.html"><img src="assets/logo.png" alt="Sphere Design" width="150" height="52"></a><p>Architectural visualization.<br>Beyond what is. Into what could be.</p><div><a href="mailto:info@thespheredesign.com">info@thespheredesign.com</a><a class="contact-phone" href="tel:+17868844880">+1 (786) 884-4880</a><span>Miami, Florida · Working worldwide</span></div></div><div class="footer-bottom"><span>© ${new Date().getFullYear()} Sphere Design</span><div>${socialIcon("Instagram", "https://www.instagram.com/thespheredesign/")}${socialIcon("LinkedIn", "https://www.linkedin.com/company/studiospheredesign/")}${socialIcon("Behance", "https://www.behance.net/spheredesignllc")}</div><a href="#top">Back to top ↑</a></div></footer>`;
...
const marquee = (large = false) => {
  const words = large
    ? '<span class="marquee-phrase">Every detail <em>speaks.</em></span><span class="marquee-medallion"><img src="assets/sphere-symbol.png" alt="" width="32" height="32"></span><span class="marquee-outline">SPHERE DESIGN</span><span class="marquee-medallion"><img src="assets/sphere-symbol.png" alt="" width="32" height="32"></span>'
  ...
  return `<section class="marquee-section ${large ? "marquee-statement" : "marquee-ribbon"}" aria-label="${large ? "Every detail speaks. Sphere Design." : "Architecture, atmosphere, materiality, light and emotion."}"><div class="marquee-topline wrap"><span>${large ? "THE SPHERE SIGNATURE" : "DESIGN / VISUALIZATION / EXPERIENCE"}</span></div><div class="marquee-window" aria-hidden="true"><div class="marquee-track"><div class="marquee-group">${words}</div><div class="marquee-group">${words}</div></div></div>${large ? `<div class="marquee-window marquee-secondary" aria-hidden="true"><div class="marquee-track"><div class="marquee-group">${secondary}${secondary}</div><div class="marquee-group">${secondary}${secondary}</div></div></div>` : ""}</section>`;
};
...
<section class="selected wrap" id="selected-work"><div class="section-heading">${eyebrow("02", "SELECTED WORK")}<h2>Unbuilt.<br><em>Unforgettable.</em></h2>${link("portfolio.html", "View full portfolio")}</div><nav class="selected-sectors" aria-label="Selected work industries">${["Residential", "Sports", "Hospitality", "Corporate", "Retail", "Food & Beverage", "Healthcare"].map(c => `<a href="portfolio.html?category=${encodeURIComponent(c)}">${c}</a>`).join("")}</nav><div class="selected-grid">${[projects[0], projects[1], projects[2], projects[3]].map(card).join("")}</div><div class="selected-sequence" hidden><div class="selected-stage">${[
    "Residential",
    "Sports",
    "Hospitality",
    "Corporate",
...
<section class="why wrap"><div class="sphere-watermark watermark-why" aria-hidden="true"><img src="assets/sphere-symbol.png" alt="" loading="lazy"></div>${eyebrow("06", "WHY SPHERE")}<h2>Design expertise.<br><em>A partner you can rely on.</em></h2><div class="why-grid">${[
    ...
    [
      "Reliable execution",
      "Clear communication, structured processes and dependable delivery. Our goal is not a one-time project, but to become a trusted long-term visualization partner for our clients.",
    ],
  ]
    .map(([a, b]) => `<article><h3>${a}</h3><p>${b}</p></article>`)
    .join("")}</div></section>
<section class="faq wrap"><div>${eyebrow("07", "A LITTLE MORE CLARITY")}<h2>Good questions.<br><em>Clear answers.</em></h2></div><div>${faqs.map(([q, a], i) => `<details><summary><span class="faq-question">${q}</span><span class="faq-toggle" aria-hidden="true"><i></i><i></i></span></summary><div class="faq-answer"><p>${a}</p></div></details>`).join("")}</div></section>${cta()}`,
};
